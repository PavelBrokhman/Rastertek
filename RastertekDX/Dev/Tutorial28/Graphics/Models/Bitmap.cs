...
namespace Engine.Graphics.Models
{
	public class Bitmap : ICloneable
	{
		...
		#region Methods
		public bool Initialize(Device device, int screeenWidth, int screenHeight, int bitmapWidth, int bitmapHeight)
		{
			return Initialize(device, screeenWidth, screenHeight, null, bitmapWidth, bitmapHeight);
		}
		public bool Initialize(Device device, int screeenWidth, int screenHeight, string textureFileName, int bitmapWidth, int bitmapHeight)
		{
			...
			if (textureFileName != null && !LoadTexture(device, textureFileName))
				return false;
