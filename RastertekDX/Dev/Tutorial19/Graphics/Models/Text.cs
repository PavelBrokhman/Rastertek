...
namespace Tutorial19.Graphics.Models
{
	public class Text
	{
		#region Structures
		...
		public bool Initialize(Device device, DeviceContext deviceContext, IntPtr windowHandle, int screanWidth, int screenHeight, Matrix baseViewMatrix)
		{
			...
			if (!UpdateSentence(ref sentences[0], "Render Count:", 20, 20, 1, 1, 1, deviceContext))
				return false;
		...
		public bool SetFps(int fps, DeviceContext deviceContext)
		{
			...
			// Update the sentence vertex buffer with the new string information.
			if (!UpdateSentence(ref sentences[0], fpsString, 20, 20, red, green, blue, deviceContext))
				return false;
		...
		public bool SetCpu(int cpu, DeviceContext deviceContext)
		{
			...
			// Update the sentence vertex buffer with the new string information.
			if (!UpdateSentence(ref sentences[1], cpuString, 20, 40, 0, 1, 0, deviceContext))
				return false;
		...
		public bool SetRenderCount(int renderCount, DeviceContext deviceContext)
		{
			...
			// Update the sentence vertex buffer with the new string information.
			if (!UpdateSentence(ref sentences[0], renderCountString, 20, 20, 1, 1, 1, deviceContext))
				return false;
