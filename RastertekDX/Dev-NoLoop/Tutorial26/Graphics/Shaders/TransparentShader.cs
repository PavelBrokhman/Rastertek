...
{
	public class TransparentShader : ICloneable
	{
		#region Structures
		...
		Buffer ConstantTransparentBuffer { get; set; }
		...
		public bool Initialize(Device device, IntPtr windowHandler)
		{
			...
			return InitializeShader(device, windowHandler, "Transparent.vs", "Transparent.ps");
		}
		...
		private bool InitializeShader(Device device, IntPtr windowHandler, string vsFileName, string psFileName)
		{
			try
			{
				...
				var vertexShaderByteCode = ShaderBytecode.CompileFromFile(vsFileName, "TransparentVertexShader", SystemConfiguration.VertexShaderProfile, ShaderFlags.None, EffectFlags.None);
				// Compile the pixel shader code.
				var pixelShaderByteCode = ShaderBytecode.CompileFromFile(psFileName, "TransparentPixelShader", SystemConfiguration.PixelShaderProfile, ShaderFlags.None, EffectFlags.None);
				...
				// Setup the description of the dynamic matrix constant buffer that is in the vertex shader.
				var TransparentBufferDesc = new BufferDescription()
				{
					Usage = ResourceUsage.Dynamic,
					SizeInBytes = Utilities.SizeOf<TransparentBuffer>(),
					BindFlags = BindFlags.ConstantBuffer,
					CpuAccessFlags = CpuAccessFlags.Write,
					OptionFlags = ResourceOptionFlags.None,
					StructureByteStride = 0
				...
				// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
				ConstantTransparentBuffer = new Buffer(device, TransparentBufferDesc);
				#endregion
		...
		private bool SetShaderParameters(DeviceContext deviceContext, Matrix worldMatrix, Matrix viewMatrix, Matrix projectionMatrix, ShaderResourceView[] textures, float blend)
		{
			try
			{
				...
				// Copy the matrices into the constant buffer.
				var TransparentBuffer = new TransparentBuffer()
				{
					blendAmount = blend
				};
				...
				// Finally set the constant buffer in the vertex shader with the updated values.
				deviceContext.PixelShader.SetConstantBuffer(bufferNumber, ConstantTransparentBuffer);
				#endregion
