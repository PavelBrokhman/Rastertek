...
{
	public class FadeShader : ICloneable
	{
		#region Structures
		...
		Buffer ConstantFadeBuffer { get; set; }
		...
		public bool Initialize(Device device, IntPtr windowHandler)
		{
			...
			return InitializeShader(device, windowHandler, "Fade.vs", "Fade.ps");
		}
		...
		private bool InitializeShader(Device device, IntPtr windowHandler, string vsFileName, string psFileName)
		{
			try
			{
				...
				var vertexShaderByteCode = ShaderBytecode.CompileFromFile(vsFileName, "FadeVertexShader", SystemConfiguration.VertexShaderProfile, ShaderFlags.None, EffectFlags.None);
				// Compile the pixel shader code.
				var pixelShaderByteCode = ShaderBytecode.CompileFromFile(psFileName, "FadePixelShader", SystemConfiguration.PixelShaderProfile, ShaderFlags.None, EffectFlags.None);
				...
				// Setup the description of the dynamic matrix constant buffer that is in the vertex shader.
				var fadeBufferDesc = new BufferDescription()
				{
					Usage = ResourceUsage.Dynamic,
					SizeInBytes = Utilities.SizeOf<FadeBuffer>(),
					BindFlags = BindFlags.ConstantBuffer,
					CpuAccessFlags = CpuAccessFlags.Write,
					OptionFlags = ResourceOptionFlags.None,
					StructureByteStride = 0
				...
				// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
				ConstantFadeBuffer = new Buffer(device, matrixBufferDesc);
				#endregion
		...
		private bool SetShaderParameters(DeviceContext deviceContext, Matrix worldMatrix, Matrix viewMatrix, Matrix projectionMatrix, ShaderResourceView texture, float fadeAmount)
		{
			try
			{
				...
				// Lock the matrices constant buffer so it can be written to.
				...
				deviceContext.PixelShader.SetConstantBuffer(bufferNumber, ConstantFadeBuffer);
				#endregion
