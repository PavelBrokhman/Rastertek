...
void CameraClass::Render()
{
    ...
    BuildViewMatrix(position, lookAt, up, 0);
...
void CameraClass::BuildViewMatrix(VectorType position, VectorType lookAt, VectorType up, int matrixType)
{
    ...
    {
        // Set the computed values in the base view matrix.
        m_baseViewMatrix[0]  = xAxis.x;
        m_baseViewMatrix[1]  = yAxis.x;
        m_baseViewMatrix[2]  = zAxis.x;
        m_baseViewMatrix[3]  = 0.0f;

        m_baseViewMatrix[4]  = xAxis.y;
        m_baseViewMatrix[5]  = yAxis.y;
        m_baseViewMatrix[6]  = zAxis.y;
        m_baseViewMatrix[7]  = 0.0f;

        m_baseViewMatrix[8]  = xAxis.z;
        m_baseViewMatrix[9]  = yAxis.z;
        m_baseViewMatrix[10] = zAxis.z;
        m_baseViewMatrix[11] = 0.0f;

        m_baseViewMatrix[12] = result1;
        m_baseViewMatrix[13] = result2;
        m_baseViewMatrix[14] = result3;
        m_baseViewMatrix[15] = 1.0f;
    }
    return;
}
...
void CameraClass::RenderBaseViewMatrix()
{
    ...
    // Finally create the view matrix from the three updated vectors.
    BuildViewMatrix(position, lookAt, up, 1);
