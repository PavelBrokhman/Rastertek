...
public unsafe class LightShader
{
    ...
    {
        public float pos1X, pos1Y, pos1Z, pad1;
        public float pos2X, pos2Y, pos2Z, pad2;
        public float pos3X, pos3Y, pos3Z, pad3;
        public float pos4X, pos4Y, pos4Z, pad4;
    }

    private struct LightColorBufferType
    {
        public float c1R, c1G, c1B, c1A;
        public float c2R, c2G, c2B, c2A;
        public float c3R, c3G, c3B, c3A;
        public float c4R, c4G, c4B, c4A;
    }
        ...
        float[] lightPositions, float[] diffuseColors)
    {
        ...
        posPtr->pos1X = lightPositions[0]; posPtr->pos1Y = lightPositions[1]; posPtr->pos1Z = lightPositions[2]; posPtr->pad1 = 1.0f;
        posPtr->pos2X = lightPositions[3]; posPtr->pos2Y = lightPositions[4]; posPtr->pos2Z = lightPositions[5]; posPtr->pad2 = 1.0f;
        posPtr->pos3X = lightPositions[6]; posPtr->pos3Y = lightPositions[7]; posPtr->pos3Z = lightPositions[8]; posPtr->pad3 = 1.0f;
        posPtr->pos4X = lightPositions[9]; posPtr->pos4Y = lightPositions[10]; posPtr->pos4Z = lightPositions[11]; posPtr->pad4 = 1.0f;
        ...
        colorPtr->c1R = diffuseColors[0]; colorPtr->c1G = diffuseColors[1]; colorPtr->c1B = diffuseColors[2]; colorPtr->c1A = diffuseColors[3];
        colorPtr->c2R = diffuseColors[4]; colorPtr->c2G = diffuseColors[5]; colorPtr->c2B = diffuseColors[6]; colorPtr->c2A = diffuseColors[7];
        colorPtr->c3R = diffuseColors[8]; colorPtr->c3G = diffuseColors[9]; colorPtr->c3B = diffuseColors[10]; colorPtr->c3A = diffuseColors[11];
        colorPtr->c4R = diffuseColors[12]; colorPtr->c4G = diffuseColors[13]; colorPtr->c4B = diffuseColors[14]; colorPtr->c4A = diffuseColors[15];
