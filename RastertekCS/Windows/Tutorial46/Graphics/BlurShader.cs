...
public unsafe class BlurShader
{
    ...
    public bool Initialize(DX11 DirectX) => InitializeShader(DirectX, "Shaders/blur.vs", "Shaders/blur.ps");
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        ComPtr<ID3D11ShaderResourceView> texture, float screenWidth, float screenHeight, float blurType)
    {
        if (!SetShaderParameters(DirectX, world, view, projection, texture, screenWidth, screenHeight, blurType)) return false;
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pVsSource, (nuint)vsSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("BlurVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pPsSource, (nuint)psSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("BlurPixelShader", NativeStringEncoding.Ansi),
