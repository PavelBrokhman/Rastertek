using RastertekCS.Windows.Tutorial23.Graphics;
using RastertekCS.Windows.Tutorial23.Inputs;
using Silk.NET.Input;
using Silk.NET.Maths;
using Silk.NET.Windowing;

namespace RastertekCS.Windows.Tutorial23.System;
...
public class SystemFramework
{
    ...
    public bool Initialize()
    {
        ...
        if (!_graphics.Initialize(_directX, _input, screenWidth, screenHeight))
            return false;
    ...
    private bool Frame()
    {
        if (_input.IsKeyDown(Key.Escape))
            return false;
        _timer.Frame();
        return _graphics.Frame(_timer.GetFrameTime());
    }
