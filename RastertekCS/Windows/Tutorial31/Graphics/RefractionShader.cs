...
public unsafe class RefractionShader
{
    ...
    private struct ClipPlaneBufferType
    {
        public Vector4D<float> clipPlane;
    }
    ...
    public bool Initialize(DX11 DirectX) =>
        InitializeShader(DirectX, "Shaders/refraction.vs", "Shaders/refraction.ps");
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        Vector3D<float> lightDirection, Vector4D<float> diffuseColor, Vector4D<float> ambientColor,
        Vector4D<float> clipPlane)
    {
        if (!SetShaderParameters(DirectX, world, view, projection, texture, lightDirection, diffuseColor, ambientColor, clipPlane))
            return false;
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(
                compiler.Compile(pVsSource, (nuint)vsSource.Length,
                    ...
                    (byte*)SilkMarshal.StringToPtr("RefractionVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(
                compiler.Compile(pPsSource, (nuint)psSource.Length,
                    ...
                    null, (ID3DInclude*)null,
                    (byte*)SilkMarshal.StringToPtr("RefractionPixelShader", NativeStringEncoding.Ansi),
        ...
        Vector4D<float> clipPlane)
    {
        ...
        var cp = (ClipPlaneBufferType*)mr.PData;
        cp->clipPlane = clipPlane;
        context.Unmap(m_clipPlaneBuffer, 0);
        var ccb = m_clipPlaneBuffer.GetPinnableReference();
