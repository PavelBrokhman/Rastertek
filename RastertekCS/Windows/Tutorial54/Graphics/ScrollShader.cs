...
public unsafe class ScrollShader
{
    ...
    public bool Initialize(DX11 DirectX) => InitializeShader(DirectX, "Shaders/scroll.vs", "Shaders/scroll.ps");
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        ComPtr<ID3D11ShaderResourceView> texture, float translation)
    {
        if (!SetShaderParameters(DirectX, world, view, projection, texture, translation)) return false;
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pVsSource, (nuint)vsSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("ScrollVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pPsSource, (nuint)psSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("ScrollPixelShader", NativeStringEncoding.Ansi),
        ...
        var translateBufferDesc = new BufferDesc { Usage = Usage.Dynamic, ByteWidth = (uint)sizeof(TranslateBufferType), BindFlags = (uint)BindFlag.ConstantBuffer, CPUAccessFlags = (uint)CpuAccessFlag.Write };
        SilkMarshal.ThrowHResult(device.CreateBuffer(&translateBufferDesc, null, ref _translateBuffer));
        ...
        ComPtr<ID3D11ShaderResourceView> texture, float translation)
    {
        ...
        var tp = (TranslateBufferType*)mr.PData;
        tp->translation = translation; tp->padding = default;
        context.Unmap(_translateBuffer, 0);
        var tcb = _translateBuffer.GetPinnableReference();
