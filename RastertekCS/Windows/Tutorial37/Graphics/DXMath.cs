...
{
    public static Matrix4X4<float> LookAtLH(Vector3D<float> eye, Vector3D<float> target, Vector3D<float> up)
    {
    ...
    public static Matrix4X4<float> OrthographicLH(float width, float height, float nearZ, float farZ)
    {
        ...
        return new Matrix4X4<float>(2f / width, 0, 0, 0, 0, 2f / height, 0, 0, 0, 0, range, 0, 0, 0, -range * nearZ, 1);
    }
}
