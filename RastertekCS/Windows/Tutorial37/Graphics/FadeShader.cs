...
public unsafe class FadeShader
{
    ...
    private ComPtr<ID3D11Buffer> _fadeBuffer;
    private ComPtr<ID3D11SamplerState> _sampleState;

    public bool Initialize(DX11 DirectX) => InitializeShader(DirectX, "Shaders/fade.vs", "Shaders/fade.ps");
    ...
    public bool Render(DX11 DirectX, int indexCount, Matrix4X4<float> worldMatrix, Matrix4X4<float> viewMatrix, Matrix4X4<float> projectionMatrix, ComPtr<ID3D11ShaderResourceView> texture, float fadeAmount)
    {
        if (!SetShaderParameters(DirectX, worldMatrix, viewMatrix, projectionMatrix, texture, fadeAmount))
            return false;
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pVsSource, (nuint)vsSource.Length, (byte*)SilkMarshal.StringToPtr(vsFilename, NativeStringEncoding.Ansi), null, (ID3DInclude*)null, (byte*)SilkMarshal.StringToPtr("FadeVertexShader", NativeStringEncoding.Ansi), (byte*)SilkMarshal.StringToPtr("vs_5_0", NativeStringEncoding.Ansi), 0, 0, &pVsBlob, &pErrorBlob));
        ComPtr<ID3D10Blob> vsBlob = pVsBlob;
        SilkMarshal.ThrowHResult(device.CreateVertexShader(vsBlob.GetBufferPointer(), vsBlob.GetBufferSize(), ref Unsafe.NullRef<ID3D11ClassLinkage>(), ref _vertexShader));

        ID3D10Blob* pPsBlob = null;
        var psSource = File.ReadAllBytes(psFilename);
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pPsSource, (nuint)psSource.Length, (byte*)SilkMarshal.StringToPtr(psFilename, NativeStringEncoding.Ansi), null, (ID3DInclude*)null, (byte*)SilkMarshal.StringToPtr("FadePixelShader", NativeStringEncoding.Ansi), (byte*)SilkMarshal.StringToPtr("ps_5_0", NativeStringEncoding.Ansi), 0, 0, &pPsBlob, &pErrorBlob));
        ComPtr<ID3D10Blob> psBlob = pPsBlob;
        SilkMarshal.ThrowHResult(device.CreatePixelShader(psBlob.GetBufferPointer(), psBlob.GetBufferSize(), ref Unsafe.NullRef<ID3D11ClassLinkage>(), ref _pixelShader));
        ...
        var fadeBufferDesc = new BufferDesc { Usage = Usage.Dynamic, ByteWidth = (uint)sizeof(FadeBufferType), BindFlags = (uint)BindFlag.ConstantBuffer, CPUAccessFlags = (uint)CpuAccessFlag.Write };
        SilkMarshal.ThrowHResult(device.CreateBuffer(&fadeBufferDesc, null, ref _fadeBuffer));
    ...
    private bool SetShaderParameters(DX11 DirectX, Matrix4X4<float> worldMatrix, Matrix4X4<float> viewMatrix, Matrix4X4<float> projectionMatrix, ComPtr<ID3D11ShaderResourceView> texture, float fadeAmount)
    {
        ...
        SilkMarshal.ThrowHResult(context.Map(_fadeBuffer, 0, Map.WriteDiscard, 0, &mr));
        var fp = (FadeBufferType*)mr.PData;
        fp->fadeAmount = fadeAmount;
        fp->padding = new Vector3D<float>(0, 0, 0);
        context.Unmap(_fadeBuffer, 0);
        var fcb = _fadeBuffer.GetPinnableReference();
        context.PSSetConstantBuffers(0, 1, &fcb);
