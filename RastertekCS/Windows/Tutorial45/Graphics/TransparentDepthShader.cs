...
public unsafe class TransparentDepthShader
{
    ...
    public bool Initialize(DX11 DirectX) => InitializeShader(DirectX, "Shaders/transparentdepth.vs", "Shaders/transparentdepth.ps");

    public void Shutdown() { _sampleState.Release(); _matrixBuffer.Release(); _layout.Release(); _pixelShader.Release(); _vertexShader.Release(); }

    public bool Render(DX11 DirectX, int indexCount, Matrix4X4<float> w, Matrix4X4<float> v, Matrix4X4<float> p,
        ComPtr<ID3D11ShaderResourceView> texture)
    {
        if (!SetShaderParameters(DirectX, w, v, p, texture)) return false;
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pVsSource, (nuint)vsSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("TransparentDepthVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pPsSource, (nuint)psSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("TransparentDepthPixelShader", NativeStringEncoding.Ansi),
    ...
    private bool SetShaderParameters(DX11 DirectX, Matrix4X4<float> w, Matrix4X4<float> v, Matrix4X4<float> p, ComPtr<ID3D11ShaderResourceView> texture)
    {
