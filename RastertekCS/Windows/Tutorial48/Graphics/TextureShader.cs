...
public unsafe class TextureShader
{
    ...
    public bool Initialize(DX11 DirectX) =>
        InitializeShader(DirectX, "Shaders/texture.vs", "Shaders/texture.ps");
    ...
    public bool Render(DX11 DirectX, int vertexCount, int instanceCount,
        Matrix4X4<float> world, Matrix4X4<float> view, Matrix4X4<float> projection,
        ComPtr<ID3D11ShaderResourceView> texture)
    {
        if (!SetShaderParameters(DirectX, world, view, projection, texture)) return false;
        RenderShader(DirectX, vertexCount, instanceCount);
        return true;
    }
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        {
            new() { SemanticName = (byte*)posName, SemanticIndex = 0, Format = Silk.NET.DXGI.Format.FormatR32G32B32Float,
                InputSlot = 0, AlignedByteOffset = 0, InputSlotClass = InputClassification.PerVertexData, InstanceDataStepRate = 0 },
            new() { SemanticName = (byte*)texName, SemanticIndex = 0, Format = Silk.NET.DXGI.Format.FormatR32G32Float,
                InputSlot = 0, AlignedByteOffset = 12, InputSlotClass = InputClassification.PerVertexData, InstanceDataStepRate = 0 },
            new() { SemanticName = (byte*)texName, SemanticIndex = 1, Format = Silk.NET.DXGI.Format.FormatR32G32B32Float,
                InputSlot = 1, AlignedByteOffset = 0, InputSlotClass = InputClassification.PerInstanceData, InstanceDataStepRate = 1 },
        };
        fixed (InputElementDesc* pLayout = layoutDesc)
            SilkMarshal.ThrowHResult(device.CreateInputLayout(pLayout, (uint)layoutDesc.Length,
                vsBlob.GetBufferPointer(), vsBlob.GetBufferSize(), ref _layout));
    ...
    private void RenderShader(DX11 DirectX, int vertexCount, int instanceCount)
    {
        var context = DirectX.DeviceContext;
        ...
        context.DrawInstanced((uint)vertexCount, (uint)instanceCount, 0, 0);
    }
}
