...
public unsafe class LightShader
{
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        ComPtr<ID3D11ShaderResourceView> colorTex, ComPtr<ID3D11ShaderResourceView> normalTex,
        Vector3D<float> lightDirection)
    {
        if (!SetShaderParameters(DirectX, world, view, projection, colorTex, normalTex, lightDirection)) return false;
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        var samplerDesc = new SamplerDesc { Filter = Filter.MinMagMipPoint, AddressU = TextureAddressMode.Wrap, AddressV = TextureAddressMode.Wrap, AddressW = TextureAddressMode.Wrap, MipLODBias = 0.0f, MaxAnisotropy = 1, ComparisonFunc = ComparisonFunc.Always, MinLOD = 0, MaxLOD = float.MaxValue };
        ...
        Vector3D<float> lightDirection)
    {
        ...
        var srvNormal = normalTex.GetPinnableReference();
        context.PSSetShaderResources(0, 1, &srvColor);
        context.PSSetShaderResources(1, 1, &srvNormal);
        return true;
    }
