...
namespace RastertekCS.Windows.Tutorial24.Graphics;

public unsafe class NormalMapShader
{
    ...
    {
        return InitializeShader(DirectX, "Shaders/NormalMap.vs", "Shaders/NormalMap.ps");
    }
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        float[] lightDirection, float[] diffuseColor)
    {
        if (!SetShaderParameters(DirectX, worldMatrix, viewMatrix, projectionMatrix, lightDirection, diffuseColor))
            return false;
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
        {
            SilkMarshal.ThrowHResult(
                compiler.Compile(
                    ...
                    (byte*)SilkMarshal.StringToPtr("NormalMapVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
        {
            SilkMarshal.ThrowHResult(
                compiler.Compile(
                    ...
                    null, (ID3DInclude*)null,
                    (byte*)SilkMarshal.StringToPtr("NormalMapPixelShader", NativeStringEncoding.Ansi),
        ...
        var layoutDesc = new InputElementDesc[]
        {
            ...
            new() { SemanticName = (byte*)normalName, SemanticIndex = 0,
                Format = Silk.NET.DXGI.Format.FormatR32G32B32Float, InputSlot = 0,
                AlignedByteOffset = 20, InputSlotClass = InputClassification.PerVertexData, InstanceDataStepRate = 0 },
            new() { SemanticName = (byte*)tangentName, SemanticIndex = 0,
                Format = Silk.NET.DXGI.Format.FormatR32G32B32Float, InputSlot = 0,
                AlignedByteOffset = 32, InputSlotClass = InputClassification.PerVertexData, InstanceDataStepRate = 0 },
            new() { SemanticName = (byte*)binormalName, SemanticIndex = 0,
                Format = Silk.NET.DXGI.Format.FormatR32G32B32Float, InputSlot = 0,
                AlignedByteOffset = 44, InputSlotClass = InputClassification.PerVertexData, InstanceDataStepRate = 0 }
        };
        ...
        float[] lightDirection, float[] diffuseColor)
    {
        ...
        lp->diffuseR = diffuseColor[0]; lp->diffuseG = diffuseColor[1]; lp->diffuseB = diffuseColor[2]; lp->diffuseA = diffuseColor[3];
        lp->lightDirX = lightDirection[0]; lp->lightDirY = lightDirection[1]; lp->lightDirZ = lightDirection[2];
        lp->padding = 0;
