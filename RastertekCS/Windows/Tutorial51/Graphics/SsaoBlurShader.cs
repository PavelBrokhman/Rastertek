...
public unsafe class SsaoBlurShader
{
    ...
    private struct ScreenBufferType
    {
    ...
    private ComPtr<ID3D11Buffer> _screenBuffer;
    private ComPtr<ID3D11SamplerState> _sampleStatePoint;

    public bool Initialize(DX11 DirectX) => InitializeShader(DirectX, "Shaders/ssaoblur.vs", "Shaders/ssaoblur.ps");
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        ComPtr<ID3D11ShaderResourceView> normalDepthTexture,
        int screenWidth, int screenHeight, int blurType)
    {
        if (!SetShaderParameters(DirectX, world, view, projection, ssaoTexture, normalDepthTexture,
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pVsSource, (nuint)vsSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("SsaoBlurVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pPsSource, (nuint)psSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("SsaoBlurPixelShader", NativeStringEncoding.Ansi),
        ...
        var screenBufferDesc = new BufferDesc { Usage = Usage.Dynamic, ByteWidth = (uint)sizeof(ScreenBufferType), BindFlags = (uint)BindFlag.ConstantBuffer, CPUAccessFlags = (uint)CpuAccessFlag.Write };
        SilkMarshal.ThrowHResult(device.CreateBuffer(&screenBufferDesc, null, ref _screenBuffer));
        ...
        int screenWidth, int screenHeight, int blurType)
    {
        ...
        SilkMarshal.ThrowHResult(context.Map(_screenBuffer, 0, Map.WriteDiscard, 0, &mr));
        var scp = (ScreenBufferType*)mr.PData;
        scp->screenWidth = screenWidth;
        scp->screenHeight = screenHeight;
        scp->blurType = blurType;
        scp->padding = 0.0f;
        context.Unmap(_screenBuffer, 0);
        var scb = _screenBuffer.GetPinnableReference();
        context.PSSetConstantBuffers(0, 1, &scb);

        var srvSsao = ssaoTexture.GetPinnableReference();
        var srvNd = normalDepthTexture.GetPinnableReference();
        context.PSSetShaderResources(0, 1, &srvSsao);
        context.PSSetShaderResources(1, 1, &srvNd);
        return true;
    }
