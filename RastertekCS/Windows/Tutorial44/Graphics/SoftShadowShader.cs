...
public unsafe class SoftShadowShader
{
    ...
    public bool Initialize(DX11 DirectX) => InitializeShader(DirectX, "Shaders/softshadow.vs", "Shaders/softshadow.ps");
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        ComPtr<ID3D11ShaderResourceView> shadowTexture,
        Vector3D<float> lightPosition, Vector4D<float> ambientColor, Vector4D<float> diffuseColor)
    {
        if (!SetShaderParameters(DirectX, world, view, projection, texture, shadowTexture,
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pVsSource, (nuint)vsSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("SoftShadowVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(compiler.Compile(pPsSource, (nuint)psSource.Length,
                ...
                (byte*)SilkMarshal.StringToPtr("SoftShadowPixelShader", NativeStringEncoding.Ansi),
        ...
        var lightPositionBufferDesc = new BufferDesc { Usage = Usage.Dynamic, ByteWidth = (uint)sizeof(LightPositionBufferType), BindFlags = (uint)BindFlag.ConstantBuffer, CPUAccessFlags = (uint)CpuAccessFlag.Write };
        ...
        Vector3D<float> lightPosition, Vector4D<float> ambientColor, Vector4D<float> diffuseColor)
    {
        ...
        var srv1 = shadowTexture.GetPinnableReference();
