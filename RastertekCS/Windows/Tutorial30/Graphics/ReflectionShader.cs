...
public unsafe class ReflectionShader
{
    ...
    {
        public Matrix4X4<float> reflectionMatrix;
    }
    ...
    {
        return InitializeShader(DirectX, "Shaders/reflection.vs", "Shaders/reflection.ps");
    }
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        ComPtr<ID3D11ShaderResourceView> texture, ComPtr<ID3D11ShaderResourceView> reflectionTexture,
        Matrix4X4<float> reflectionMatrix)
    {
        if (!SetShaderParameters(DirectX, worldMatrix, viewMatrix, projectionMatrix, texture, reflectionTexture, reflectionMatrix))
            return false;
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(
                compiler.Compile(pVsSource, (nuint)vsSource.Length,
                    ...
                    (byte*)SilkMarshal.StringToPtr("ReflectionVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(
                compiler.Compile(pPsSource, (nuint)psSource.Length,
                    ...
                    null, (ID3DInclude*)null,
                    (byte*)SilkMarshal.StringToPtr("ReflectionPixelShader", NativeStringEncoding.Ansi),
        ...
        SilkMarshal.ThrowHResult(device.CreateBuffer(&reflectionBufferDesc, null, ref m_reflectionBuffer));
        ...
        Matrix4X4<float> reflectionMatrix)
    {
        ...
        reflectionMatrix = Matrix4X4.Transpose(reflectionMatrix);
        ...
        rp->reflectionMatrix = reflectionMatrix;
        ...
        var srv1 = reflectionTexture.GetPinnableReference();
