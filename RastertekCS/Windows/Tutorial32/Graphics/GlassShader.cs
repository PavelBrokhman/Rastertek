...
public unsafe class GlassShader
{
    ...
    private struct RefractionScaleBufferType
    {
    ...
    private ComPtr<ID3D11Buffer> m_refractionScaleBuffer;
    private ComPtr<ID3D11SamplerState> m_sampleState;

    public bool Initialize(DX11 DirectX) =>
        InitializeShader(DirectX, "Shaders/glass.vs", "Shaders/glass.ps");

    public void Shutdown()
    {
        m_sampleState.Release();
        m_refractionScaleBuffer.Release();
        ...
        float refractionScale)
    {
        if (!SetShaderParameters(DirectX, world, view, projection,
                colorTexture, normalTexture, refractionTexture, refractionScale))
            return false;
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(
                compiler.Compile(pVsSource, (nuint)vsSource.Length,
                    ...
                    (byte*)SilkMarshal.StringToPtr("GlassVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(
                compiler.Compile(pPsSource, (nuint)psSource.Length,
                    ...
                    null, (ID3DInclude*)null,
                    (byte*)SilkMarshal.StringToPtr("GlassPixelShader", NativeStringEncoding.Ansi),
        ...
        var layoutDesc = new InputElementDesc[]
        {
            ...
            new() { SemanticName = (byte*)normName, SemanticIndex = 0,
                Format = Silk.NET.DXGI.Format.FormatR32G32B32Float, InputSlot = 0,
                AlignedByteOffset = 20, InputSlotClass = InputClassification.PerVertexData, InstanceDataStepRate = 0 }
        };
        ...
        SilkMarshal.Free(posName); SilkMarshal.Free(texName); SilkMarshal.Free(normName);
        ...
        var refScaleDesc = new BufferDesc
        {
            Usage = Usage.Dynamic,
            ByteWidth = (uint)sizeof(RefractionScaleBufferType),
            BindFlags = (uint)BindFlag.ConstantBuffer,
            CPUAccessFlags = (uint)CpuAccessFlag.Write,
            MiscFlags = 0, StructureByteStride = 0
        };
        SilkMarshal.ThrowHResult(device.CreateBuffer(&refScaleDesc, null, ref m_refractionScaleBuffer));
        ...
        var samplerDesc = new SamplerDesc
        {
            ...
            MipLODBias = 0.0f, MaxAnisotropy = 1,
        ...
        float refractionScale)
    {
        ...
        SilkMarshal.ThrowHResult(context.Map(m_refractionScaleBuffer, 0, Map.WriteDiscard, 0, &mr));
        var rp = (RefractionScaleBufferType*)mr.PData;
        rp->refractionScale = refractionScale;
        rp->padding1 = rp->padding2 = rp->padding3 = 0;
        context.Unmap(m_refractionScaleBuffer, 0);
        var rcb = m_refractionScaleBuffer.GetPinnableReference();
        context.PSSetConstantBuffers(0, 1, &rcb);

        var ct = colorTexture.GetPinnableReference();
        context.PSSetShaderResources(0, 1, &ct);
        var nt = normalTexture.GetPinnableReference();
        context.PSSetShaderResources(1, 1, &nt);
        var rt = refractionTexture.GetPinnableReference();
        context.PSSetShaderResources(2, 1, &rt);
