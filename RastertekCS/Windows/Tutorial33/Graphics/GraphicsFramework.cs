...
public class GraphicsFramework
{
    ...
    public bool Initialize(DX11 DirectX, int screenWidth, int screenHeight)
    {
        ...
        m_Model = new Model();
        if (!m_Model.Initialize(DirectX, "Models/square.txt", "Data/fire01.tga", false)) return false;

        m_FireTexture  = new Texture(); if (!m_FireTexture.Initialize(DirectX,  "Data/fire01.tga",   false)) return false;
        m_NoiseTexture = new Texture(); if (!m_NoiseTexture.Initialize(DirectX, "Data/noise01.tga",  true))  return false;
        m_AlphaTexture = new Texture(); if (!m_AlphaTexture.Initialize(DirectX, "Data/alpha01.tga",  false)) return false;

        m_FireShader = new FireShader();
        if (!m_FireShader.Initialize(DirectX)) return false;
    ...
    {
        m_frameTime += 0.01f;
        if (m_frameTime > 1000.0f) m_frameTime = 0.0f;
        return Render();
    }
    ...
    private bool Render()
    {
        ...
        if (!m_FireShader.Render(m_DirectX, m_Model.GetIndexCount(), world, view, projection,
                m_frameTime, scrollSpeeds, scales, d1, d2, d3, 0.8f, 0.5f,
