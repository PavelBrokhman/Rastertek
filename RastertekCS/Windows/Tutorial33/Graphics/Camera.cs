...
public class Camera
{
    ...
    private Matrix4X4<float> _reflectionViewMatrix;
    ...
    {
        var up = new Vector3D<float>(0.0f, 1.0f, 0.0f);
        var position = new Vector3D<float>(_positionX, -_positionY + height * 2.0f, _positionZ);
        var lookAt = new Vector3D<float>(0.0f, 0.0f, 1.0f);
        ...
        lookAt = position + lookAt;
        _reflectionViewMatrix = DXMath.LookAtLH(position, lookAt, up);
    }

    public Matrix4X4<float> GetReflectionViewMatrix() => _reflectionViewMatrix;
}
