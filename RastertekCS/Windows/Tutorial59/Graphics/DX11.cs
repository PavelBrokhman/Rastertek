...
public unsafe class DX11
{
    ...
    private ComPtr<ID3D11BlendState> _blendEnableState;
    private ComPtr<ID3D11BlendState> _blendDisableState;
    private ComPtr<ID3D11BlendState> _blendParticleEnableState;
    ...
    )
    {
        ...
        // d3dclass.cpp keeps a second state with DepthEnable off, which the
        // particles are drawn with - otherwise the first quad at z = 0 writes
        // depth and every quad behind it fails the test.
        var depthDisabledDesc = depthStencilDesc;
        depthDisabledDesc.DepthEnable = false;
        SilkMarshal.ThrowHResult(
            _device.CreateDepthStencilState(&depthDisabledDesc, ref _depthDisabledStencilState)
        );
        ...
        SilkMarshal.ThrowHResult(_device.CreateBlendState(&blendDesc, ref _blendEnableState));

        // d3dclass.cpp keeps a third state just for the particles: additive on
        // colour like the one above, but the alpha channel is handled
        // differently - SrcBlendAlpha ZERO and DestBlendAlpha INV_SRC_ALPHA.
        blendDesc.RenderTarget[0].SrcBlendAlpha = Blend.Zero;
        blendDesc.RenderTarget[0].DestBlendAlpha = Blend.InvSrcAlpha;
        SilkMarshal.ThrowHResult(_device.CreateBlendState(&blendDesc, ref _blendParticleEnableState));

        blendDesc.RenderTarget[0].SrcBlendAlpha = Blend.One;
        blendDesc.RenderTarget[0].DestBlendAlpha = Blend.Zero;
        blendDesc.RenderTarget[0].BlendEnable = false;
        SilkMarshal.ThrowHResult(_device.CreateBlendState(&blendDesc, ref _blendDisableState));
    ...
    public void EnableAlphaBlending()
    {
        ...
        _deviceContext.OMSetBlendState(_blendEnableState, bf, 0xFFFFFFFF);
    }
    ...
    {
        float* bf = stackalloc float[4] { 0, 0, 0, 0 };
        _deviceContext.OMSetBlendState(_blendParticleEnableState, bf, 0xFFFFFFFF);
    }

    public void DisableAlphaBlending()
    {
        float* bf = stackalloc float[4] { 0, 0, 0, 0 };
        _deviceContext.OMSetBlendState(_blendDisableState, bf, 0xFFFFFFFF);
    }

    public void Shutdown()
    {
        _deviceContext.ClearState();
        _blendDisableState.Release();
        _blendParticleEnableState.Release();
        _blendEnableState.Release();
