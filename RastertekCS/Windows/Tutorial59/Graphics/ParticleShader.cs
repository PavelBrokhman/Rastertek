...
public unsafe class ParticleShader
{
    ...
    public bool Initialize(DX11 DirectX) => InitializeShader(DirectX, "Shaders/particle.vs", "Shaders/particle.ps");
    ...
    public bool Render(DX11 DirectX, int indexCount, Matrix4X4<float> worldMatrix, Matrix4X4<float> viewMatrix, Matrix4X4<float> projectionMatrix, ComPtr<ID3D11ShaderResourceView> texture)
    {
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        // particle.vs takes per-particle life time and scroll in data1 : TEXCOORD1,
        // where Tut38 carried a colour.
        var colName = SilkMarshal.StringToPtr("TEXCOORD", NativeStringEncoding.Ansi);
        var layoutDesc = new InputElementDesc[]
        {
            ...
            new() { SemanticName = (byte*)colName, SemanticIndex = 1, Format = Silk.NET.DXGI.Format.FormatR32G32B32A32Float, InputSlot = 0, AlignedByteOffset = 20, InputSlotClass = InputClassification.PerVertexData, InstanceDataStepRate = 0 },
        };
    ...
    private bool SetShaderParameters(DX11 DirectX, Matrix4X4<float> worldMatrix, Matrix4X4<float> viewMatrix, Matrix4X4<float> projectionMatrix, ComPtr<ID3D11ShaderResourceView> texture)
    {
