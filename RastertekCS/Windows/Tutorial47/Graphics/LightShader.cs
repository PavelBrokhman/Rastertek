...
public unsafe class LightShader
{
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        Vector3D<float> lightDirection, Vector4D<float> diffuseColor)
    {
        if (!SetShaderParameters(DirectX, world, view, projection, texture, lightDirection, diffuseColor)) return false;
