...
public unsafe class TransparentShader
{
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        ComPtr<ID3D11ShaderResourceView> texture, float blendAmount)
    {
        if (!SetShaderParameters(DirectX, worldMatrix, viewMatrix, projectionMatrix, texture, blendAmount))
            return false;
        ...
        fixed (byte* pVsSource = vsSource)
            SilkMarshal.ThrowHResult(
                compiler.Compile(pVsSource, (nuint)vsSource.Length,
                    ...
                    (byte*)SilkMarshal.StringToPtr("TransparentVertexShader", NativeStringEncoding.Ansi),
        ...
        fixed (byte* pPsSource = psSource)
            SilkMarshal.ThrowHResult(
                compiler.Compile(pPsSource, (nuint)psSource.Length,
                    ...
                    null, (ID3DInclude*)null,
                    (byte*)SilkMarshal.StringToPtr("TransparentPixelShader", NativeStringEncoding.Ansi),
        ...
        ComPtr<ID3D11ShaderResourceView> texture, float blendAmount)
    {
        ...
        tp->blendAmount = blendAmount; tp->pad0 = 0; tp->pad1 = 0; tp->pad2 = 0;
