...
public unsafe class SpecMapShader
{
    ...
    public bool Render(DX11 DirectX, int indexCount,
        ...
        float[] lightDirection, float[] diffuseColor, float[] cameraPosition,
    ...
    private bool InitializeShader(DX11 DirectX, string vsFilename, string psFilename)
    {
        ...
        fixed (byte* pVsSource = vsSource)
        {
            SilkMarshal.ThrowHResult(
                compiler.Compile(
                    ...
                    (byte*)SilkMarshal.StringToPtr("SpecMapVertexShader", NativeStringEncoding.Ansi),
        ...
        SilkMarshal.ThrowHResult(
            device.CreateBuffer(&cameraBufferDesc, null, ref m_cameraBuffer));
        ...
        float[] specularColor, float specularPower)
    {
        ...
        cp->camX = cameraPosition[0]; cp->camY = cameraPosition[1]; cp->camZ = cameraPosition[2];
        cp->padding = 0;
        context.Unmap(m_cameraBuffer, 0);

        var ccb = m_cameraBuffer.GetPinnableReference();
        ...
        lp->specularR = specularColor[0]; lp->specularG = specularColor[1]; lp->specularB = specularColor[2]; lp->specularA = specularColor[3];
