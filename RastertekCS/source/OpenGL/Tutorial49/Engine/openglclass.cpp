...
bool OpenGLClass::Initialize(Display* display, Window win, int screenWidth, int screenHeight, float screenNear, float screenDepth, bool vsync)
{
    ...
    // Render in wireframe so the tessellated sub-triangles are visible.
    // This matches the DX11 Tutorial49 rasterizer state (D3D11_FILL_WIREFRAME).
...
bool OpenGLClass::LoadExtensionList()
{
    ...
    // Load the tessellation entry point for setting the patch vertex count.
    glPatchParameteri = (PFNGLPATCHPARAMETERIPROC)glXGetProcAddress((unsigned char*)"glPatchParameteri");
