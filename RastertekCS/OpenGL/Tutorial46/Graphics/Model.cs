...
public class Model
{
    ...
    public unsafe bool Initialize(GL4 OpenGL, string modelFilename, string textureFilename1, string textureFilename2)
    {
        if (!LoadModel(modelFilename)) return false;
        if (!InitializeBuffers(OpenGL)) return false;
        _texture1 = new Texture();
        if (!_texture1.Initialize(OpenGL, textureFilename1, 0, true)) return false;
        _texture2 = new Texture();
        if (!_texture2.Initialize(OpenGL, textureFilename2, 0, true)) return false;
        return true;
    }

    public void Shutdown(GL4 OpenGL)
    {
        _texture2?.Shutdown(OpenGL); _texture2 = null;
        _texture1?.Shutdown(OpenGL); _texture1 = null;
    ...
    public void SetTexture1(GL4 OpenGL, uint textureUnit) => _texture1?.SetTexture(OpenGL, textureUnit);
    public void SetTexture2(GL4 OpenGL, uint textureUnit) => _texture2?.SetTexture(OpenGL, textureUnit);
