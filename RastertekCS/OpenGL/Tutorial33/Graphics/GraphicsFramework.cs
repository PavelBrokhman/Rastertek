...
public class GraphicsFramework
{
    ...
    { m_gl.BeginScene(0, 0, 0, 1); var w = m_gl.GetWorldMatrix(); var v = m_cam.GetViewMatrix(); var p = m_gl.GetProjectionMatrix();
      float[] ss = { 1.3f, 2.1f, 2.3f }; float[] sc = { 1f, 2f, 3f }; float[] d1 = { 0.1f, 0.2f }; float[] d2 = { 0.1f, 0.3f }; float[] d3 = { 0.1f, 0.1f };
      m_gl.EnableAlphaBlending();
      m_fireShader.SetShaderParameters(m_gl, w, v, p, m_frameTime, ss, sc, d1, d2, d3, 0.8f, 0.5f);
      m_model.SetTexture1(m_gl, 0); m_model.SetTexture2(m_gl, 1); m_model.SetTexture3(m_gl, 2); m_model.Render(m_gl);
      m_gl.DisableAlphaBlending(); m_gl.EndScene(); return true; }
}
