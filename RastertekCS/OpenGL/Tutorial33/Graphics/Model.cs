...
public class Model
{
    ...
    )
    {
        ...
        if (texture3File != null)
        {
            _textures[2] = new Texture();
            if (!_textures[2].Initialize(gl, texture3File, 0, wrap3))
                return false;
        }
        return true;
    }
    ...
    public void SetTexture3(GL4 gl, uint textureUnit)
    {
        _textures?[2]?.SetTexture(gl, textureUnit);
    }
