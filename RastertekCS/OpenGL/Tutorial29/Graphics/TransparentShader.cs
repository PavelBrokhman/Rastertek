...
public class TransparentShader
{
    ...
    public bool Initialize(GL4 OpenGL) => InitializeShader(OpenGL, "Shaders/Transparent.vs", "Shaders/Transparent.ps");
    ...
    public unsafe bool SetShaderParameters(GL4 OpenGL,
        ...
        float blendAmount, int textureUnit)
    {
        var gl = OpenGL.Gl;
        int loc;
        ...
        loc = gl.GetUniformLocation(m_shaderProgram, "blendAmount");
        if (loc == -1) return false;
        gl.Uniform1(loc, blendAmount);
