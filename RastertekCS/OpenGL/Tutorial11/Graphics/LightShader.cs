...
public class LightShader
{
    ...
    public bool SetShaderParameters(GL4 OpenGL,
        ...
        int textureUnit, float[] lightPositions, float[] diffuseColors)
    {
        var gl = OpenGL.Gl;
        int loc;
        ...
        if (loc == -1) return false;
        unsafe { fixed (float* p = lightPositions) gl.Uniform3(loc, NUM_LIGHTS, p); }

        // Массив из NUM_LIGHTS vec4 цветов.
        loc = gl.GetUniformLocation(m_shaderProgram, "diffuseColor");
        if (loc == -1) return false;
        unsafe { fixed (float* p = diffuseColors) gl.Uniform4(loc, NUM_LIGHTS, p); }
    ...
    private static bool CheckShaderCompile(GL gl, uint shader, string filename)
    {
        ...
        if (s != 1) { global::System.Console.WriteLine($"Compile {filename}: {gl.GetShaderInfoLog(shader)}"); return false; }
        return true;
    }
}
