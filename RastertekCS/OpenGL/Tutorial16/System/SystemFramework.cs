using RastertekCS.OpenGL.Tutorial16.Graphics;
using RastertekCS.OpenGL.Tutorial16.Inputs;
using Silk.NET.Input;
using Silk.NET.Maths;
using Silk.NET.Windowing;

namespace RastertekCS.OpenGL.Tutorial16.System;
...
public class SystemFramework
{
    ...
    private void OnLoad()
    {
        ...
        {
            mouse.MouseMove += (_, pos) => m_Input?.ProcessMouse((int)pos.X, (int)pos.Y);
            mouse.MouseDown += (_, btn) => m_Input?.MouseDown();
            mouse.MouseUp += (_, btn) => m_Input?.MouseUp();
        }
    }
    ...
    private void OnRender(double dt)
    {
        if (!m_graphicsInitialized)
            return;
        if (m_done || m_Input.IsKeyDown(Key.Escape))
        {
            m_done = true;
            m_window.Close();
            return;
        }
        m_Input.GetMouseLocation(out int mx, out int my);
        if (!m_Graphics.Frame(mx, my, m_Input.IsMousePressed()))
        {
            m_done = true;
            m_window.Close();
        }
