...
public class LightShader
{
        ...
        float[] cameraPosition, float[] specularColor, float specularPower)
    {
        ...
        loc = gl.GetUniformLocation(m_shaderProgram, "cameraPosition");
        if (loc == -1) return false;
        unsafe { fixed (float* p = cameraPosition) gl.Uniform3(loc, 1, p); }
        ...
        if (loc == -1) return false;
        unsafe { fixed (float* p = lightDirection) gl.Uniform3(loc, 1, p); }

        loc = gl.GetUniformLocation(m_shaderProgram, "diffuseLightColor");
        if (loc == -1) return false;
        unsafe { fixed (float* p = diffuseLightColor) gl.Uniform4(loc, 1, p); }

        loc = gl.GetUniformLocation(m_shaderProgram, "ambientLight");
        if (loc == -1) return false;
        unsafe { fixed (float* p = ambientLight) gl.Uniform4(loc, 1, p); }

        loc = gl.GetUniformLocation(m_shaderProgram, "specularColor");
        if (loc == -1) return false;
        unsafe { fixed (float* p = specularColor) gl.Uniform4(loc, 1, p); }

        loc = gl.GetUniformLocation(m_shaderProgram, "specularPower");
        if (loc == -1) return false;
        gl.Uniform1(loc, specularPower);
    ...
    private bool InitializeShader(GL4 OpenGL, string vsFilename, string psFilename)
    {
        ...
        if (ls != 1) { global::System.Console.WriteLine($"Ошибка линковки: {gl.GetProgramInfoLog(m_shaderProgram)}"); return false; }
        return true;
    }
    ...
    private static bool CheckShaderCompile(GL gl, uint shader, string filename)
    {
        ...
        if (s != 1) { global::System.Console.WriteLine($"Ошибка компиляции {filename}: {gl.GetShaderInfoLog(shader)}"); return false; }
        return true;
    }
}
