...
public class ShadowShader
{
    ...
    )
    {
        ...
        loc = gl.GetUniformLocation(_shaderProgram, "lightViewMatrix");
        if (loc == -1) return false;
        gl.UniformMatrix4(loc, 1, false, (float*)&lightViewMatrix);
        loc = gl.GetUniformLocation(_shaderProgram, "lightProjectionMatrix");
        if (loc == -1) return false;
        gl.UniformMatrix4(loc, 1, false, (float*)&lightProjectionMatrix);
        loc = gl.GetUniformLocation(_shaderProgram, "shaderTexture");
        if (loc == -1) return false;
        gl.Uniform1(loc, 0);
        loc = gl.GetUniformLocation(_shaderProgram, "depthMapTexture");
        if (loc == -1) return false;
        gl.Uniform1(loc, 1);
        ...
        loc = gl.GetUniformLocation(_shaderProgram, "bias");
        if (loc == -1) return false;
