...
public class ProjectionShader
{
    ...
    public bool Initialize(GL4 OpenGL) => InitializeShader(OpenGL, "Shaders/Projection.vs", "Shaders/Projection.ps");
    ...
    )
    {
        ...
        loc = gl.GetUniformLocation(_shaderProgram, "viewMatrix2");
        if (loc == -1) return false;
        gl.UniformMatrix4(loc, 1, false, (float*)&viewMatrix2);
        loc = gl.GetUniformLocation(_shaderProgram, "projectionMatrix2");
        if (loc == -1) return false;
        gl.UniformMatrix4(loc, 1, false, (float*)&projectionMatrix2);
        loc = gl.GetUniformLocation(_shaderProgram, "shaderTexture");
        if (loc == -1) return false;
        gl.Uniform1(loc, 0);
        loc = gl.GetUniformLocation(_shaderProgram, "projectionTexture");
        if (loc == -1) return false;
        gl.Uniform1(loc, 1);
        loc = gl.GetUniformLocation(_shaderProgram, "diffuseColor");
        if (loc == -1) return false;
        gl.Uniform4(loc, 1, (float*)&diffuseColor);
        loc = gl.GetUniformLocation(_shaderProgram, "ambientColor");
        if (loc == -1) return false;
        gl.Uniform4(loc, 1, (float*)&ambientColor);
        loc = gl.GetUniformLocation(_shaderProgram, "lightPosition");
        if (loc == -1) return false;
        gl.Uniform3(loc, 1, (float*)&lightPosition);
        loc = gl.GetUniformLocation(_shaderProgram, "brightness");
        if (loc == -1) return false;
        gl.Uniform1(loc, brightness);
        return true;
    }
