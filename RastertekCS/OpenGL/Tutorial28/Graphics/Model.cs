...
public class Model
{
    ...
    public unsafe bool Initialize(GL4 OpenGL, string modelFilename, string textureFilename, uint textureUnit, bool wrap)
    {
        if (!LoadModel(modelFilename)) return false;
        if (!InitializeBuffers(OpenGL)) return false;
        m_Texture1 = new Texture();
        if (!m_Texture1.Initialize(OpenGL, textureFilename, textureUnit, wrap)) return false;
        return true;
    }
    ...
    public void SetTexture(GL4 OpenGL, uint textureUnit) { m_Texture1?.SetTexture(OpenGL, textureUnit); }
    ...
    private bool LoadModel(string filename)
    {
        ...
        int vertexCount = 0; int dataStart = -1;
        ...
        m_vertexCount = vertexCount; m_indexCount = vertexCount;
        ...
        for (int i = dataStart; i < lines.Length && vi < vertexCount; i++)
        {
            ...
            m_modelData[off + 4] = float.Parse(parts[4]);
    ...
    private unsafe bool InitializeBuffers(GL4 OpenGL)
    {
        ...
        {
            int off = i * 8;
            vertices[i].x = m_modelData[off + 0]; vertices[i].y = m_modelData[off + 1]; vertices[i].z = m_modelData[off + 2];
            vertices[i].tu = m_modelData[off + 3]; vertices[i].tv = m_modelData[off + 4];
            vertices[i].nx = m_modelData[off + 5]; vertices[i].ny = m_modelData[off + 6]; vertices[i].nz = m_modelData[off + 7];
            indices[i] = (uint)i;
        }
