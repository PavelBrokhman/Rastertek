...
public class Model
{
    ...
    public unsafe bool Initialize(GL4 OpenGL, string modelFilename, string textureFilename, bool wrap)
    {
        if (!LoadModel(modelFilename)) return false;
        if (!InitializeBuffers(OpenGL)) return false;
        m_Texture = new Texture();
        if (!m_Texture.Initialize(OpenGL, textureFilename, 0, wrap)) return false;
        return true;
    }
    ...
    private unsafe bool InitializeBuffers(GL4 OpenGL)
    {
        ...
        {
            int off = i * 8;
            vertices[i].x = m_modelData[off]; vertices[i].y = m_modelData[off + 1]; vertices[i].z = m_modelData[off + 2];
