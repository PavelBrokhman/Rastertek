...
public class Texture
{
    ...
    public unsafe bool Initialize(GL4 OpenGL, string filename, uint textureUnit, bool wrap)
    {
        ...
        { Console.WriteLine($"Failed to load texture: {filename}"); return false; }
    ...
    private static bool LoadTga(string filename, out int width, out int height, out byte[] rgba)
    {
        ...
        width = data[12] | (data[13] << 8); height = data[14] | (data[15] << 8);
        if (imageType != 2 || (bpp != 24 && bpp != 32)) return false;
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            int srcOff = offset + srcRow * width * channels, dstOff = y * width * 4;
    ...
    private static void GenerateCheckerboardTga(string filename, int size)
    {
        ...
        for (int y = 0; y < size; y++)
            for (int x = 0; x < size; x++)
            {
                ...
                int i = (y * size + x) * 4;
                pixels[i] = lit ? (byte)200 : (byte)20; pixels[i + 1] = lit ? (byte)200 : (byte)20;
                pixels[i + 2] = lit ? (byte)200 : (byte)20; pixels[i + 3] = 255;
            }
        using var fs = File.Create(filename); fs.Write(header); fs.Write(pixels);
    }
}
