...
public class Camera
{
    ...
    private Matrix4X4<float> m_viewMatrix, m_baseViewMatrix;
    ...
    public void Render()
    {
        ...
        var pos = new Vector3D<float>(m_positionX, m_positionY, m_positionZ);
        var lookAt = new Vector3D<float>(0, 0, 1);
        var rot = Matrix4X4.CreateFromYawPitchRoll(m_rotationY * (MathF.PI / 180f), m_rotationX * (MathF.PI / 180f), m_rotationZ * (MathF.PI / 180f));
        lookAt = Vector3D.Transform(lookAt, rot); up = Vector3D.Transform(up, rot);
        lookAt = pos + lookAt;
        m_viewMatrix = Matrix4X4.CreateLookAt(pos, lookAt, up);
    }
    ...
    public void RenderBaseViewMatrix()
    {
        ...
        lookAt = Vector3D.Transform(lookAt, rot); up = Vector3D.Transform(up, rot);
        lookAt = pos + lookAt;
        m_baseViewMatrix = Matrix4X4.CreateLookAt(pos, lookAt, up);
    }

    public Matrix4X4<float> GetViewMatrix() => m_viewMatrix;
    public Matrix4X4<float> GetBaseViewMatrix() => m_baseViewMatrix;
}
