...
public class FadeShader
{
    ...
    public bool Initialize(GL4 OpenGL) => InitializeShader(OpenGL, "Shaders/Fade.vs", "Shaders/Fade.ps");
    ...
    )
    {
        ...
        if (loc == -1) return false;
        gl.Uniform1(loc, 0);
        loc = gl.GetUniformLocation(_shaderProgram, "fadeAmount");
        if (loc == -1) return false;
        gl.Uniform1(loc, fadeAmount);
        return true;
    }
