...
public class ScrollShader
{
    ...
    public bool Initialize(GL4 OpenGL) => InitializeShader(OpenGL, "Shaders/Scroll.vs", "Shaders/Scroll.ps");
    ...
    public unsafe bool SetShaderParameters(GL4 OpenGL, Matrix4X4<float> world, Matrix4X4<float> view, Matrix4X4<float> projection, float textureTranslation)
    {
        var gl = OpenGL.Gl;
        ...
        int loc = gl.GetUniformLocation(m_shaderProgram, "worldMatrix"); if (loc == -1) Console.WriteLine("warn: worldMatrix");
        gl.UniformMatrix4(loc, 1, false, (float*)&tpWorld);
        loc = gl.GetUniformLocation(m_shaderProgram, "viewMatrix"); if (loc == -1) Console.WriteLine("warn: viewMatrix");
        gl.UniformMatrix4(loc, 1, false, (float*)&tpView);
        loc = gl.GetUniformLocation(m_shaderProgram, "projectionMatrix"); if (loc == -1) Console.WriteLine("warn: projectionMatrix");
        gl.UniformMatrix4(loc, 1, false, (float*)&tpProj);
        loc = gl.GetUniformLocation(m_shaderProgram, "shaderTexture"); if (loc == -1) Console.WriteLine("warn: shaderTexture"); gl.Uniform1(loc, 0);
        loc = gl.GetUniformLocation(m_shaderProgram, "textureTranslation"); if (loc == -1) Console.WriteLine("warn: textureTranslation"); gl.Uniform1(loc, textureTranslation);
        return true;
    }
