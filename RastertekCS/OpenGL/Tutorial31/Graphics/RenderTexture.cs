...
{
    private uint m_fbo, m_texId, m_depthId;
    ...
    {
        var gl = OpenGL.Gl; m_tw = tw; m_th = th;
        m_fbo = gl.GenFramebuffer(); gl.BindFramebuffer(FramebufferTarget.Framebuffer, m_fbo);
        gl.ActiveTexture(TextureUnit.Texture0);
        m_texId = gl.GenTexture(); gl.BindTexture(TextureTarget.Texture2D, m_texId);
        gl.TexImage2D(TextureTarget.Texture2D, 0, (int)InternalFormat.Rgba, (uint)tw, (uint)th, 0, PixelFormat.Rgba, PixelType.UnsignedByte, null);
        gl.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMagFilter, (int)TextureMagFilter.Linear);
        gl.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.Linear);
        gl.FramebufferTexture2D(FramebufferTarget.Framebuffer, FramebufferAttachment.ColorAttachment0, TextureTarget.Texture2D, m_texId, 0);
        m_depthId = gl.GenRenderbuffer(); gl.BindRenderbuffer(RenderbufferTarget.Renderbuffer, m_depthId);
    ...
    public void Shutdown(GL4 OpenGL) { var gl = OpenGL.Gl; gl.DeleteRenderbuffer(m_depthId); gl.DeleteTexture(m_texId); gl.DeleteFramebuffer(m_fbo); }
    public void SetRenderTarget(GL4 OpenGL) { OpenGL.Gl.BindFramebuffer(FramebufferTarget.Framebuffer, m_fbo); OpenGL.Gl.Viewport(0, 0, (uint)m_tw, (uint)m_th); }
