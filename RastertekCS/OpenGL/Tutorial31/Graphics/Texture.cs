...
public class Texture
{
    ...
    {
        w = 0; h = 0; rgba = null;
        byte[] d = File.ReadAllBytes(fn); if (d.Length < 18) return false;
        int idLen = d[0], it = d[2], bpp = d[16], desc = d[17];
        w = d[12] | (d[13] << 8); h = d[14] | (d[15] << 8);
        if (it != 2 || (bpp != 24 && bpp != 32)) return false;
        int off = 18 + idLen, ch = bpp / 8, pc = w * h;
    ...
    private static void GenerateCheckerboardTga(string fn, int sz)
    {
        ...
        byte cr = 180, cg = 180, cb = 180;
        var nm = Path.GetFileNameWithoutExtension(fn).ToLower();
        if (nm.Contains("ground")) { cr = 80; cg = 140; cb = 60; }
        else if (nm.Contains("wall")) { cr = 200; cg = 180; cb = 160; }
        else if (nm.Contains("marble")) { cr = 220; cg = 220; cb = 230; }
        else if (nm.Contains("water")) { cr = 40; cg = 80; cb = 200; }
        ...
        for (int y = 0; y < sz; y++) for (int x = 0; x < sz; x++) { bool lit = (((x / cell) + (y / cell)) & 1) == 0; int i = (y * sz + x) * 4; px[i] = lit ? cb : (byte)20; px[i + 1] = lit ? cg : (byte)20; px[i + 2] = lit ? cr : (byte)20; px[i + 3] = 255; }
