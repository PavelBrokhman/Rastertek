using RastertekCS.OpenGL.Tutorial31.Graphics;
using RastertekCS.OpenGL.Tutorial31.Inputs;
using Silk.NET.Input;
using Silk.NET.Maths;
using Silk.NET.Windowing;

namespace RastertekCS.OpenGL.Tutorial31.System;
...
public class SystemFramework
{
    ...
    private bool InitializeWindows(ref int sw, ref int sh)
    {
        ...
        m_window.Load += OnLoad; m_window.Render += OnRender; m_window.Closing += OnClosing;
        m_window.Initialize(); sw = m_window.Size.X; sh = m_window.Size.Y;
        m_window.Title = "Tutorial31";
        return true;
    }
    ...
    private void OnLoad()
    {
        ...
        foreach (var kb in m_inputContext.Keyboards) { kb.KeyDown += (_, key, _) => m_Input?.KeyDown(key); kb.KeyUp += (_, key, _) => m_Input?.KeyUp(key); }
    }
