...
public class BlurShader
{
        ...
        float screenWidth, float screenHeight, float blurType)
    {
        ...
        loc = gl.GetUniformLocation(m_shaderProgram, "screenWidth"); if (loc == -1) Console.WriteLine($"warn: uniform not found"); gl.Uniform1(loc, screenWidth);
        loc = gl.GetUniformLocation(m_shaderProgram, "screenHeight"); if (loc == -1) Console.WriteLine($"warn: uniform not found"); gl.Uniform1(loc, screenHeight);
        loc = gl.GetUniformLocation(m_shaderProgram, "blurType"); if (loc == -1) Console.WriteLine($"warn: uniform not found"); gl.Uniform1(loc, blurType);
        return true;
    }
