...
public class SsaoBlurShader
{
    private uint m_vertexShader, m_fragmentShader, m_shaderProgram;

    public bool Initialize(GL4 OpenGL) => InitializeShader(OpenGL, "Shaders/SsaoBlur.vs", "Shaders/SsaoBlur.ps");
        ...
        float screenWidth, float screenHeight, float blurType)
    {
        ...
        loc = gl.GetUniformLocation(m_shaderProgram, "ssaoTexture"); if (loc == -1) return false; gl.Uniform1(loc, 0);
        loc = gl.GetUniformLocation(m_shaderProgram, "normalDepthTexture"); if (loc == -1) return false; gl.Uniform1(loc, 1);
        ...
        loc = gl.GetUniformLocation(m_shaderProgram, "blurType"); if (loc == -1) return false; gl.Uniform1(loc, blurType);
        return true;
    }
