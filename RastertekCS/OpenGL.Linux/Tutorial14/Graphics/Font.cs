...
public class Font
{
    ...
    public bool Initialize(GL4 OpenGL, string fontDataFile, string fontTextureFile, uint textureUnit)
    {
        if (!File.Exists(fontDataFile) || !File.Exists(fontTextureFile))
            GenerateDefaultFont(fontDataFile, fontTextureFile);

        if (!LoadFontData(fontDataFile)) return false;

        m_Texture = new Texture();
        if (!m_Texture.Initialize(OpenGL, fontTextureFile, textureUnit, false)) return false;
    ...
    public void SetTexture(GL4 OpenGL, uint textureUnit) => m_Texture?.SetTexture(OpenGL, textureUnit);
    ...
    public int GetSentencePixelLength(string sentence)
    {
        ...
        {
            int idx = c - 32;
            if (idx >= 0 && idx < m_Font.Length) length += m_Font[idx].Size;
        }
        return length;
    }
    ...
    public void BuildVertexArray(float[] vertices, string sentence, float drawX, float drawY)
    {
        ...
        {
            int letter = c - 32;
            if (letter < 0 || letter >= m_Font.Length) letter = 0;
            ...
            if (c != ' ')
            {
                ...
                vertices[idx++] = tl;    vertices[idx++] = tt;
                vertices[idx++] = right; vertices[idx++] = bottom; vertices[idx++] = 0;
                vertices[idx++] = tr;    vertices[idx++] = tb;
                vertices[idx++] = left;  vertices[idx++] = bottom; vertices[idx++] = 0;
                vertices[idx++] = tl;    vertices[idx++] = tb;
                // Tri 2
                vertices[idx++] = left;  vertices[idx++] = top;    vertices[idx++] = 0;
                vertices[idx++] = tl;    vertices[idx++] = tt;
                vertices[idx++] = right; vertices[idx++] = top;    vertices[idx++] = 0;
                vertices[idx++] = tr;    vertices[idx++] = tt;
                vertices[idx++] = right; vertices[idx++] = bottom; vertices[idx++] = 0;
                vertices[idx++] = tr;    vertices[idx++] = tb;
            }
    ...
    private static void GenerateDefaultFont(string dataFile, string textureFile)
    {
        ...
        if (!string.IsNullOrEmpty(texDir)) Directory.CreateDirectory(texDir);

        int texSize = 256, cols = 16, cellW = texSize / cols, cellH = 16;
        ...
        using (var w = new StreamWriter(dataFile))
        {
            for (int i = 0; i < 95; i++)
            {
                ...
                w.WriteLine($"{ascii} {ch} {leftU.ToString("F6", inv)} {rightU.ToString("F6", inv)} {cellW - 2}");
            }
        }
        ...
        header[2] = 2;
        header[12] = (byte)(texSize & 0xFF); header[13] = (byte)((texSize >> 8) & 0xFF);
        header[14] = (byte)(texH & 0xFF); header[15] = (byte)((texH >> 8) & 0xFF);
        ...
        for (int i = 0; i < 95; i++)
        {
            ...
            for (int y = 2; y < cellH - 2; y++)
                for (int x = 2; x < cellW - 2; x++)
                {
                    ...
                    px[pi + 0] = 255; px[pi + 1] = 255; px[pi + 2] = 255; px[pi + 3] = 255;
                }
        }

        using var fs = File.Create(textureFile);
        fs.Write(header); fs.Write(px);
    }
}
