...
public class SpecMapShader
{
    ...
    public bool Initialize(GL4 OpenGL) => InitializeShader(OpenGL, "Shaders/SpecMap.vs", "Shaders/SpecMap.ps");
    ...
    public unsafe bool SetShaderParameters(GL4 OpenGL,
        ...
        int textureUnit1, int textureUnit2, int textureUnit3)
    {
        var gl = OpenGL.Gl;
        int loc;
        ...
        loc = gl.GetUniformLocation(m_shaderProgram, "shaderTexture1");
        if (loc == -1) return false;
        gl.Uniform1(loc, textureUnit1);

        loc = gl.GetUniformLocation(m_shaderProgram, "shaderTexture2");
        if (loc == -1) return false;
        gl.Uniform1(loc, textureUnit2);

        loc = gl.GetUniformLocation(m_shaderProgram, "shaderTexture3");
        if (loc == -1) return false;
        ...
        if (loc == -1) return false;
        fixed (float* p = specularColor) gl.Uniform4(loc, 1, p);
