...
public class WaterShader
{
    ...
    public unsafe bool SetShaderParameters(GL4 gl, Matrix4X4<float> world, Matrix4X4<float> view, Matrix4X4<float> proj,
        Matrix4X4<float> reflection, float waterTrans, float reflRefScale)
    {
        ...
        loc = g.GetUniformLocation(m_prog, "reflectionMatrix"); if (loc >= 0) g.UniformMatrix4(loc, 1, false, (float*)&reflection);
        loc = g.GetUniformLocation(m_prog, "normalTexture"); if (loc >= 0) g.Uniform1(loc, 0);
        loc = g.GetUniformLocation(m_prog, "refractionTexture"); if (loc >= 0) g.Uniform1(loc, 1);
        loc = g.GetUniformLocation(m_prog, "reflectionTexture"); if (loc >= 0) g.Uniform1(loc, 2);
        loc = g.GetUniformLocation(m_prog, "waterTranslation"); if (loc >= 0) g.Uniform1(loc, waterTrans);
        loc = g.GetUniformLocation(m_prog, "reflectRefractScale"); if (loc >= 0) g.Uniform1(loc, reflRefScale);
        return true;
    }
    ...
    private bool Init(GL4 gl, string vsf, string psf)
    {
        ...
        g.BindAttribLocation(m_prog, 0, "inputPosition"); g.BindAttribLocation(m_prog, 1, "inputTexCoord");
