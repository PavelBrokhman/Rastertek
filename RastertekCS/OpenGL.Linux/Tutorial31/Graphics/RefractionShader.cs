...
public class RefractionShader
{
    ...
    public unsafe bool SetShaderParameters(GL4 gl, Matrix4X4<float> world, Matrix4X4<float> view, Matrix4X4<float> proj,
        float[] lightDir, float[] diffuse, float[] ambient, float[] clipPlane)
    {
        ...
        loc = g.GetUniformLocation(m_prog, "clipPlane"); if (loc >= 0) fixed (float* p = clipPlane) g.Uniform4(loc, 1, p);
