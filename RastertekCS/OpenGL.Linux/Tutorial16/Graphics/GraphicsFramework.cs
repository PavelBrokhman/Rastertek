...
public class GraphicsFramework
{
    ...
    public bool Initialize(GL4 OpenGL, int screenWidth, int screenHeight)
    {
        ...
        if (!m_MouseStrings[0].Initialize(OpenGL, m_Font, "Mouse X: 0", 10, 10, 1, 1, 1, screenWidth, screenHeight, 32)) return false;
        m_MouseStrings[1] = new Text();
        if (!m_MouseStrings[1].Initialize(OpenGL, m_Font, "Mouse Y: 0", 10, 35, 1, 1, 1, screenWidth, screenHeight, 32)) return false;
        m_MouseStrings[2] = new Text();
        if (!m_MouseStrings[2].Initialize(OpenGL, m_Font, "Mouse Button: No", 10, 60, 1, 1, 1, screenWidth, screenHeight, 32)) return false;
    ...
    public void Shutdown()
    {
        ...
        {
            foreach (var t in m_MouseStrings) t?.Shutdown(m_OpenGL);
            m_MouseStrings = null;
        }
    ...
    {
        m_MouseStrings[0].UpdateText(m_OpenGL, m_Font, $"Mouse X: {mouseX}", 10, 10, 1, 1, 1, m_screenWidth, m_screenHeight);
        m_MouseStrings[1].UpdateText(m_OpenGL, m_Font, $"Mouse Y: {mouseY}", 10, 35, 1, 1, 1, m_screenWidth, m_screenHeight);
        m_MouseStrings[2].UpdateText(m_OpenGL, m_Font, mouseDown ? "Mouse Button: Yes" : "Mouse Button: No", 10, 60, 1, 1, 1, m_screenWidth, m_screenHeight);
        return true;
    }
    ...
    private bool Render()
    {
        ...
        if (!m_FontShader.SetShaderParameters(m_OpenGL, world, view, ortho, (int)TEXTURE_UNIT, color)) return false;
