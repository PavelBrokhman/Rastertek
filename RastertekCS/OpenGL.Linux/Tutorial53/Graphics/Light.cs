namespace RastertekCS.OpenGL.Tutorial53.Graphics;

public class Light
{
    private float[] m_diffuseColor = new float[4];
    private float[] m_direction = new float[3];
    private float[] m_ambientLight = new float[4];

    public void SetDiffuseColor(float r, float g, float b, float a) { m_diffuseColor[0]=r; m_diffuseColor[1]=g; m_diffuseColor[2]=b; m_diffuseColor[3]=a; }
    public void SetDirection(float x, float y, float z) { m_direction[0]=x; m_direction[1]=y; m_direction[2]=z; }
    public void SetAmbientLight(float r, float g, float b, float a) { m_ambientLight[0]=r; m_ambientLight[1]=g; m_ambientLight[2]=b; m_ambientLight[3]=a; }

    public float[] GetDiffuseColor() => (float[])m_diffuseColor.Clone();
    public float[] GetDirection() => (float[])m_direction.Clone();
    public float[] GetAmbientLight() => (float[])m_ambientLight.Clone();
}
