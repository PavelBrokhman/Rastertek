...
public class OrthoWindow
{
    ...
    {
        var gl = OpenGL.Gl;
        float left = -(windowWidth / 2.0f), right = left + windowWidth;
        float top = windowHeight / 2.0f, bottom = top - windowHeight;
        m_indexCount = 6;
        ...
        fixed (VertexType* p = vertices) gl.BufferData(BufferTargetARB.ArrayBuffer, (nuint)(sizeof(VertexType) * 6), p, BufferUsageARB.StaticDraw);
        ...
        fixed (uint* p = indices) gl.BufferData(BufferTargetARB.ElementArrayBuffer, (nuint)(sizeof(uint) * 6), p, BufferUsageARB.StaticDraw);
        return true;
    }
