...
public class GraphicsFramework
{
    ...
    public bool Initialize(GL4 OpenGL, int screenWidth, int screenHeight)
    {
        ...
        m_FpsText = new Text();
        if (!m_FpsText.Initialize(OpenGL, m_Font, "FPS: 0", 10, 10, 0, 1, 0, screenWidth, screenHeight, 32)) return false;
    ...
    {
        m_FpsText?.Shutdown(m_OpenGL); m_FpsText = null;
    ...
    public bool Frame(int fps)
    {
        if (fps != m_previousFps)
        {
            m_previousFps = fps;
            float r = 0, g = 1, b = 0;
            if (fps < 60) { r = 1; g = 1; b = 0; }
            if (fps < 30) { r = 1; g = 0; b = 0; }
            m_FpsText.UpdateText(m_OpenGL, m_Font, $"FPS: {fps}", 10, 10, r, g, b, m_screenWidth, m_screenHeight);
        }
        return Render();
    }
    ...
    private bool Render()
    {
        ...
        if (!m_FontShader.SetShaderParameters(m_OpenGL, world, view, ortho, (int)TEXTURE_UNIT, m_FpsText.GetPixelColor())) return false;
