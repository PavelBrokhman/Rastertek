...
public class ProjectionShader
{
    ...
    public bool Initialize(GL4 gl4) => Init(gl4, "Shaders/Projection.vs", "Shaders/Projection.ps");
    ...
    public bool SetShaderParameters(GL4 gl4, Matrix4X4<float> w, Matrix4X4<float> v, Matrix4X4<float> p, Matrix4X4<float> v2, Matrix4X4<float> p2)
    {
        ...
        loc=g.GetUniformLocation(m_prog,"viewMatrix2"); if(loc==-1)return false; unsafe{g.UniformMatrix4(loc,1,false,(float*)&v2);}
        loc=g.GetUniformLocation(m_prog,"projectionMatrix2"); if(loc==-1)return false; unsafe{g.UniformMatrix4(loc,1,false,(float*)&p2);}
        loc=g.GetUniformLocation(m_prog,"shaderTexture"); if(loc==-1)return false; g.Uniform1(loc,0);
        loc=g.GetUniformLocation(m_prog,"projectionTexture"); if(loc==-1)return false; g.Uniform1(loc,1);
        return true;
    }
